...
contract supply {
    ...
    function retBuyProd(
        ...
        require(
            !prodList[prodID].isSold,
            "Product already sold to a customer!"
        );
        require(
            mapRet[msg.sender].isRet,
            "Only retialer can perform this action!"
        );

        prod storage product = prodList[prodID];
        supplier storage sup = supList[supplyID];

        uint256 priceForRetailer = product.price +
            ((product.price * sup.percentCharged) / 100);
        require(priceForRetailer <= msg.value, "Insufficient wallet balance!");
        payable(product.manufacturer).transfer(product.price);
        payable(sup.addr).transfer(
            ((product.price * sup.percentCharged) / 100)
        );
    ...
    function getAllProducts() public view returns (prod[] memory products) {
        products = new prod[](prodCount);

        for (uint256 i = 0; i < prodCount; i++) {
            products[i] = prodList[i];
        }
    ...
    function getAllSuppliers()
        public
        view
        returns (supplier[] memory suppliers)
    {
        suppliers = new supplier[](supplyCount);

        for (uint256 i = 0; i < supplyCount; i++) {
            suppliers[i] = supList[i];
        }
